...
public class attendancePage extends JFrame {
    ...
    private void loadStudentsFromDatabase() {
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT Username FROM student WHERE Course_code = ?");
...
public class attendancePage extends JFrame {
    ...
    private void loadComboBoxData() {
        try (Connection con = DBConnection.getConnection()) {
            // Load course codes from Course_unit table
            //TO ID
            comboBox1.removeAllItems();
            ...
            // Load TO usernames from Technical_officer table
            comboBox3.removeAllItems();
            PreparedStatement ps3 = con.prepareStatement("SELECT DISTINCT Session_Type FROM Attendance");
            ResultSet rs3 = ps3.executeQuery();
            while (rs3.next()) comboBox3.addItem(rs3.getString("Username"));
...
public class attendancePage extends JFrame {
    ...
    private void loadComboBoxData() {
        try (Connection con = DBConnection.getConnection()) {
            ...
            // ComboBox 3 - Course Type (Session Type)
            comboBox3.removeAllItems();
            PreparedStatement ps3 = con.prepareStatement("SELECT DISTINCT Session_Type FROM Attendance");
            ResultSet rs3 = ps3.executeQuery();
            while (rs3.next()) {
                comboBox3.addItem(rs3.getString("Session_Type"));
            }
    ...
    private void calculateAttendanceEligibility() {
        try (Connection con = DBConnection.getConnection()) {
            String query = "SELECT * FROM attendance_eligibility_view";
            PreparedStatement ps = con.prepareStatement(query);
            ResultSet rs = ps.executeQuery();
